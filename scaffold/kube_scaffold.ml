...
let render_resource definition =
  ...
  let source =
    String.concat "\n"
      ([
         "(* Generated by ocaml-k8s scaffold. Edit the model and reconcile \
...
Generated by `ocaml-k8s scaffold` from `%s` for `%s/%s`, Kind `%s`.
...
let render_init_resource definition =
  ...
  in
  Printf.sprintf
    {|(* Generated by ocaml-k8s init. These OCaml types are the source of truth
...
Generated by `ocaml-k8s init` as a type-first operator for `%s/%s`, Kind `%s`.
...
let generate_files ~output files =
  if path_exists output then Error ("output path already exists: " ^ output)
    ...
    if not (path_exists parent) then
      ...
      let temporary =
        output ^ ".ocaml-k8s-tmp-" ^ string_of_int (Unix.getpid ())
      in
