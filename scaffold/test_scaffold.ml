...
let test_init_render () =
  ...
  Alcotest.(check bool)
    ...
    (contains resource "Generated by ocaml-kube init");
  Alcotest.(check bool)
...
let test_generate () =
  let parent = Filename.temp_dir "ocaml-kube-scaffold-" "" in
  Fun.protect
...
let test_validation () =
  ...
  in
  let path = Filename.temp_file "ocaml-kube-crd-" ".json" in
...
let () =
  Alcotest.run "ocaml-kube scaffold"
    [
      ( "project",
        [
